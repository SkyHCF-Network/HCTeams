...
public class SetBalCommand {
    ...
    @Command(names = {"addmoney", "addbal"}, permission = "foxtrot.setbal")
    public static void addBal(CommandSender sender, @Param(name = "target") Player target, @Param(name = "amount") Integer amount){
        FrozenEconomyHandler.setBalance(target.getUniqueId(), FrozenEconomyHandler.getBalance(target.getUniqueId()) + amount);
        sender.sendMessage(BukkitChat.format("&aAdded $" + amount + " to " + target.getName() + "'s balance."));
    }
