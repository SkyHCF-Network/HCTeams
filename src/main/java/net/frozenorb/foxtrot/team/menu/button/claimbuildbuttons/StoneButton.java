...
public class StoneButton extends Button {
    public void clicked(final Player player, final int i, final ClickType clickType) {
        ...
        new BlockRunnable(BlockRunnable.BlockOperation.X, Material.STONE, minY, min, max, 50).runTaskTimer((Plugin) Foxtrot.getInstance(), 0L, 1L);
        new BlockRunnable(BlockRunnable.BlockOperation.Z, Material.STONE, minY, min, max, 50).runTaskTimer((Plugin) Foxtrot.getInstance(), 0L, 1L);
    }

    public String getName(final Player player) {
        return ChatColor.RED + "Generate Stone Box";
    }
