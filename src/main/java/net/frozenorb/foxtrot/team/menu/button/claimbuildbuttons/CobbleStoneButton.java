package net.frozenorb.foxtrot.team.menu.button.claimbuildbuttons;
...
public class CobbleStoneButton extends Button {
    public void clicked(final Player player, final int i, final ClickType clickType) {
        ...
        new BlockRunnable(BlockRunnable.BlockOperation.X, Material.COBBLESTONE, 50, min, max, 1).runTaskTimer((Plugin) Foxtrot.getInstance(), 0L, 1L);
        new BlockRunnable(BlockRunnable.BlockOperation.Z, Material.COBBLESTONE, 50, min, max, 1).runTaskTimer((Plugin) Foxtrot.getInstance(), 0L, 1L);
    }

    public String getName(final Player player) {
        return ChatColor.RED + "Generate Cobblestone Box";
    }
