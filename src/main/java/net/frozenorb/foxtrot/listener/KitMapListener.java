...
public class KitMapListener implements Listener {
    ...
    public void onPlayerDeath(PlayerDeathEvent e) {
        ...
        if (victim.getKiller() != null && !victim.getUniqueId().equals(victim.getKiller().getUniqueId()) && !Players.isNaked(victim)) {
            ...
            Bukkit.getScheduler().runTask(Foxtrot.getInstance(), () -> {
                ...
                if (kills % 10 == 0) {
                    Bukkit.getServer().dispatchCommand(Bukkit.getConsoleSender(), "crate givekey " + killerName + " centile 1");
                    victim.getKiller().sendMessage(ChatColor.GREEN + "You received an Centile key for 10 kills!");
                }
            });
        }
    }
    ...
    public void onPlayerInteract(PlayerInteractEvent event) {
        ...
        if(!sign.getLine(0).startsWith(ChatColor.BLUE + "- Kit")) return;
        Kit kit = Foxtrot.getInstance().getMapHandler().getKitManager().get(player.getUniqueId(), sign.getLine(1));
        event.getPlayer().sendMessage((CC.Color("&aYou have applied the &e" + sign.getLine(1) + " &akit.")));
        attemptApplyKit(player, kit);
    }

    private void attemptApplyKit(Player player, Kit kit) {
    }
